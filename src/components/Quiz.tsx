...
export default function Quiz({ phone, phoneDisplay }: Props) {
    ...
    return (
      <div className="rounded-3xl bg-white ring-1 ring-black/5 shadow-sm p-7 max-w-md mx-auto text-center">
        ...
        <button
          type="button"
          onClick={() => setStep(0)}
          className="mt-6 inline-flex items-center justify-center gap-2 rounded-full bg-[#7d7635] text-white px-7 py-3.5 font-bold text-sm hover:bg-[#5e5a25] transition-colors shadow-md shadow-[#7d7635]/20"
        >
          Start <ChevronRight size={16} />
        </button>
        <a
          href={`tel:${phone}`}
          className="block mt-4 text-[#7d7635] text-sm font-semibold underline-offset-4 hover:underline"
        >
  ...
  if (step >= STEPS.length) {
    return (
      <div className="rounded-3xl bg-white ring-1 ring-black/5 shadow-sm p-7 max-w-md mx-auto text-center">
        <span className="grid place-items-center w-12 h-12 mx-auto mb-4 rounded-full bg-[#7d7635]/10 text-[#7d7635]">
        ...
        <a
          href={`tel:${phone}`}
          className="mt-6 inline-flex items-center justify-center gap-2 rounded-full bg-[#7d7635] text-white px-7 py-4 font-bold text-base hover:bg-[#5e5a25] transition-colors shadow-md shadow-[#7d7635]/20 w-full"
        >
          <Phone size={18} />
        ...
        <button
          type="button"
          onClick={reset}
          className="mt-4 inline-flex items-center gap-1.5 text-[#2c2c2c] text-sm font-semibold hover:text-[#7d7635] transition-colors"
        >
  ...
  return (
    <div className="rounded-3xl bg-white ring-1 ring-black/5 shadow-sm p-7 max-w-md mx-auto">
      <div className="flex items-center justify-between mb-5 gap-4">
        <span className="text-[#7d7635] text-[11px] tracking-[0.25em] uppercase font-bold">
          Step {step + 1} of {STEPS.length}
        </span>
        <a
          href={`tel:${phone}`}
          className="inline-flex items-center gap-1 text-[#7d7635] text-xs font-bold underline-offset-4 hover:underline whitespace-nowrap"
        >
      ...
      {/* progress bar */}
      <div className="h-1.5 w-full rounded-full bg-[#ffffff] overflow-hidden mb-6">
        <div
          className="h-full bg-[#7d7635] transition-all duration-500"
        ...
        {current.options.map((option) => (
          <li key={option}>
            <button
              ...
              className="w-full text-left rounded-2xl bg-[#ffffff] hover:bg-[#7d7635] hover:text-white ring-1 ring-black/5 hover:ring-[#7d7635] px-5 py-4 text-[15px] font-semibold text-[#1a1a1a] transition-colors"
            >
              {option}
            </button>
          </li>
